...
if (-not $env:SHADI_POLICY_PATH) {
	$env:SHADI_POLICY_PATH = Join-Path $rootDir 'policies/demo/avatar.json'
}
if (-not $env:SHADI_PYTHON) {
	$env:SHADI_PYTHON = Join-Path $rootDir '.venv\Scripts\python.exe'
}
if (-not $env:SLIM_TLS_CERT) {
	$env:SLIM_TLS_CERT = Join-Path $env:SHADI_TMP_DIR 'shadi-slim-mtls/client-avatar.crt'
}
if (-not $env:SLIM_TLS_KEY) {
	$env:SLIM_TLS_KEY = Join-Path $env:SHADI_TMP_DIR 'shadi-slim-mtls/client-avatar.key'
}
...
try {
	uv run --no-project --python $env:SHADI_PYTHON tools/run_sandboxed_agent.py --policy $env:SHADI_POLICY_PATH -- $env:SHADI_PYTHON agents/avatar/adk_agent/run_shadi_memory.py
}
finally {
	Pop-Location
}