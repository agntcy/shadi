...
struct LiveSlimGroupSender {
    ...
    channel_name: Arc<Name>,
    participants: Vec<Arc<Name>>,
...
impl LiveSlimGroupSender {
    fn new(config: LiveSlimGroupConfig) -> Result<Self, String> {
        ...
        if !config.receipt_files.is_empty() && config.participants.len() != config.receipt_files.len() {
            return Err(format!(
        ...
        let session = app
            ...
            .map_err(format_slim_error)?;
        let mut participant_refs = Vec::with_capacity(config.participants.len());
        for participant in &config.participants {
            let participant_ref = Arc::new(parse_slim_name(participant)?);
            app.set_route(participant_ref.clone(), connection_id)
                .map_err(format_slim_error)?;
            session
                .invite_and_wait(participant_ref.clone())
                .map_err(format_slim_error)?;
            participant_refs.push(participant_ref);
        }
        ...
        Ok(Self {
            ...
            connection_id,
            subscriptions: vec![local_name_ref, channel_name_ref.clone()],
    ...
    fn create_group_session(&self) -> Result<Arc<Session>, String> {
        let session = self
            .app
            .create_session_and_wait(group_session_config(), self.channel_name.clone())
            .map_err(format_slim_error)?;
        for participant in &self.participants {
            self.app
                .set_route(participant.clone(), self.connection_id)
                .map_err(format_slim_error)?;
            session
                .invite_and_wait(participant.clone())
                .map_err(format_slim_error)?;
        }
        Ok(session)
    }

    fn is_transient_publish_error(message: &str) -> bool {
        message.contains("Session already closed")
            || message.contains("Session closed")
            || message.contains("dropped")
            || message.contains("data plane is shutting down")
    ...
    fn publish(
        ...
        let started_at = Instant::now();
        let max_retries = std::env::var("SHADI_LIVE_SLIM_PUBLISH_RETRY_ATTEMPTS")
            .ok()
            .and_then(|raw| raw.parse::<usize>().ok())
            .unwrap_or(3);
        let retry_backoff_ms = std::env::var("SHADI_LIVE_SLIM_PUBLISH_RETRY_BACKOFF_MS")
            .ok()
            .and_then(|raw| raw.parse::<u64>().ok())
            .filter(|value| *value > 0)
            .unwrap_or(1000);
        ...
        for attempt in 0..=max_retries {
            let publish_result = {
                let session = self
                    .session
                    .lock()
                    .map_err(|_| "live SLIM group sender session lock poisoned".to_string())?;
                session.publish_and_wait(
                    payload.to_vec(),
                    payload_type.clone(),
                    Some(HashMap::new()),
                )
            ...
            match publish_result {
                Ok(()) => break,
                Err(err) => {
                    let message = format_slim_error(err);
                    if attempt == max_retries || !Self::is_transient_publish_error(&message) {
                        return Err(message);
                    }

                    let replacement = self.create_group_session().map_err(|session_err| {
                        format!(
                            "{}; failed to recreate SLIM group session (attempt {}/{}): {}",
                            message,
                            attempt + 1,
                            max_retries,
                            session_err
        ...
        let receipts = if self.receipt_files.is_empty() {
            vec!["group publish acknowledged by transport".to_string()]
        } else {
            wait_for_group_receipts(
                &self.receipt_files,
                round_index,
...
impl Drop for LiveSlimGroupSender {
    fn drop(&mut self) {
        if let Ok(session) = self.session.lock() {
            let _ = self.app.delete_session_and_wait(session.clone());
        }
