...
fn snapshot_test_command() -> (Vec<String>, PathBuf) {
    #[cfg(target_os = "macos")]
    {
        (vec!["/usr/bin/true".to_string()], PathBuf::from("/usr/bin"))
    }
...
fn resolve_policy_skips_missing_preset_paths() {
    ...
    let resolved = resolve_policy(&cli, &policy_file)
        .expect("resolve should succeed with missing preset paths");
    ...
    assert!(
        !resolved
            ...
            .allow_read()
            .iter()
            .any(|p| p.to_string_lossy().contains("does/not/exist")),
...
fn format_policy_includes_net_allow() {
    let policy = SandboxPolicy::new().allow_network_destination("10.0.0.1:443");
    let blocked = HashSet::new();
    let allow = HashSet::new();

    let output = format_policy(&policy, &blocked, &allow).expect("format");
    assert!(
        output.contains("\"net_allow\""),
        "output should contain net_allow: {}",
        output
    );
...
fn format_policy_sorts_commands() {
    ...
    let allow = ["zsh".to_string(), "bash".to_string()]
        .into_iter()
        .collect();
...
fn secret_action_deserializes_kebab_case_values() {
    let action: SecretAction =
        serde_json::from_str("\"delegate-to-child\"").expect("deserialize secret action");
...
fn load_policy_file_parses_json() {
    ...
    assert_eq!(
        policy.process_secret_policy[0].secret,
        "secops/github_token"
    );
    assert_eq!(
        policy.process_secret_policy[0].actions,
        vec![SecretAction::DelegateToChild]
    );
    assert_eq!(
        policy.process_secret_policy[0].children,
...
fn git_snapshot_session_writes_artifact_without_sandbox() {
    ...
    assert_eq!(
        artifact["layout"]["root_dir"],
        snapshot_dir.display().to_string()
    );
    assert_eq!(
        artifact["layout"]["latest_file"],
        snapshot_dir.join("latest.json").display().to_string()
    );
    ...
    assert!(
        artifact["git"]["before"]["hashes"]["state_sha256"]
            .as_str()
    ...
    assert!(
        artifact["git"]["after"]["hashes"]["state_sha256"]
            .as_str()
    ...
    let latest =
        std::fs::read_to_string(snapshot_dir.join("latest.json")).expect("read latest artifact");
...
fn canonicalize_helpers_resolve_paths() {
    ...
    let text =
        shadi_sandbox::canonicalize_path(dir.path().to_str().expect("str")).expect("str path");
    assert_eq!(path, text);
}
...
fn read_openpgp_input_errors_when_secret_key_is_missing() {
    let err =
        read_openpgp_input("--key", Some(&unique_key("openpgp/missing-secret")), None).unwrap_err();
...
fn list_keychain_with_store_filters_prefix() {
    ...
    store
        .put("secops/a", b"1", SecretPolicy::default())
        .unwrap();
...
fn inject_keychain_with_store_sets_env() {
    let store = MemoryStore::new();
    store
        .put("secops/token", b"value", SecretPolicy::default())
        .unwrap();
    ...
    assert!(envs.iter().any(|(key, value)| {
        *key == std::ffi::OsStr::new("TOKEN") && *value == Some(std::ffi::OsStr::new("value"))
    }));
}
...
fn inject_keychain_with_store_reports_missing_key() {
    let store = MemoryStore::new();
    let mut command = Command::new("/usr/bin/true");
    let err = inject_keychain_with_store(&store, &mut command, &["missing=TOKEN".to_string()])
        .unwrap_err();
    assert!(err.contains("keychain lookup failed"));
}
...
fn inject_keychain_with_store_rejects_invalid_mapping() {
    ...
    let mut command = Command::new("/usr/bin/true");
    let err =
        inject_keychain_with_store(&store, &mut command, &["invalid".to_string()]).unwrap_err();
...
fn inject_keychain_secrets_uses_default_store() {
    ...
    assert!(envs.iter().any(|(env_key, value)| {
        *env_key == std::ffi::OsStr::new("TOKEN") && *value == Some(std::ffi::OsStr::new("value"))
    }));
}
...
fn policy_scoped_keychain_rule_only_applies_to_matching_process() {
    ...
    let resolved =
        resolve_launch_secret_config(&command, &cli, &policy).expect("resolve secret config");
    ...
    assert!(!envs
        .iter()
        .any(|(env_key, _)| *env_key == std::ffi::OsStr::new("SHOULD_NOT_APPLY")));
}
...
fn run_cli_print_policy_with_invalid_cli_paths_returns_error() {
    ...
    cli.run_command.clear();
    cli.read
        .push(PathBuf::from("/this/path/does/not/exist/for-shadi"));
...
fn run_cli_verify_agent_identity_invalid_public_key_encoding_returns_error() {
    ...
    test_store_put(
        &format!("agents/{}/public", agent_name),
        b"%%%not-base64%%%",
    );
...
fn run_cli_ssh_and_seed_sources_are_not_interchangeable() {
    ...
    for source in [HumanIdentitySource::Ssh, HumanIdentitySource::Seed] {
        ...
        assert_eq!(code, ExitCode::from(0));
        dids.push(
            String::from_utf8(test_store_get(&format!("{prefix}/{agent}/did")).unwrap()).unwrap(),
        );
    }
...
fn extract_ed25519_public_key_from_cert() {
    let public_key =
        extract_ed25519_public_key(&sample_openpgp_cert_armored()).expect("extract key");
...
fn trace_span_name_reads_span_name() {
    ...
    assert_eq!(
        trace_span_name(&value),
        Some("shadi.sandbox.run".to_string())
    );
}
...
fn trace_list_errors_on_missing_file() {
    let err = trace_list(Path::new("/tmp/does-not-exist.jsonl"), 5, None, None, None).unwrap_err();
...
"#,
    ...
    let config_path = write_mas_config(dir.path(), &config);
...
fn read_memory_payload_rejects_conflicting_inputs() {
    let err = read_memory_payload(
        Some("inline".to_string()),
        Some(PathBuf::from("payload.txt")),
    )
    .unwrap_err();
